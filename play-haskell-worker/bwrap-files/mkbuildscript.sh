...
function build_depends_for() {
  case "$1" in
    8.6.5)
      ...
      ;;
    9.6.*)
      echo "array, base, binary, bytestring, Cabal, containers, deepseq, directory, exceptions, filepath, ghc-boot, ghc-boot-th, ghc-heap, ghci, ghc-prim, haskeline, hpc, integer-gmp, libiserv, mtl, parsec, pretty, primitive, process, random, rts, stm, template-haskell, terminfo, text, time, transformers, transformers-base, unix, vector, xhtml, extra, bitvec, tagged, vector-algorithms, transformers-compat, linear-base, linear-generics, storable-tuple, text-builder-linear"
      ;;
    *)
