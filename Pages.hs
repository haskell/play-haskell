...
data Pages = Pages { pIndex :: ByteString
                   , pResponse :: (ByteString, ByteString, ByteString)
                   , pPasteRead :: (ByteString, ByteString, ByteString) }

pagesFromDisk :: IO Pages
pagesFromDisk = Pages <$> readIndexHTML <*> readResponseHTML <*> readPasteReadHTML

readIndexHTML :: IO ByteString
readIndexHTML = Char8.pack <$> readFile "index.html"

readResponseHTML :: IO (ByteString, ByteString, ByteString)
readResponseHTML = do
    fileContents <- readFile "response.html"
